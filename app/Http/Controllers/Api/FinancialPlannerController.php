...
class FinancialPlannerController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $monthlyBudget = (float) Budget::where('user_id', $uid)->get()->sum(fn($b) => match ($b->period) {'weekly' => (float)$b->amount * 52 / 12, 'annually' => (float)$b->amount / 12, default => (float)$b->amount});
        ...
        $profile = FinancialPlannerProfile::firstOrCreate(['user_id' => $uid], ['current_age'=>30,'retirement_age'=>60,'current_retirement_savings'=>0,'expected_annual_return'=>5,'inflation_rate'=>3,'retirement_years'=>20]);
        ...
        $future = ($r > 0 && $n > 0) ? $starting * pow(1 + $r, $n) + $contribution * ((pow(1 + $r, $n) - 1) / $r) : $starting + $contribution * $n;
        ...
        return response()->json(['profile'=>$profile,'metrics'=>$metrics,'start_date'=>$start->toDateString(),'end_date'=>$end->toDateString()]);
    }

    public function update(Request $request): JsonResponse
    {
        $data = $request->validate([
            'current_age'=>'required|integer|min:18|max:100','retirement_age'=>'required|integer|min:18|max:110|gt:current_age',
            'current_retirement_savings'=>'required|numeric|min:0','monthly_retirement_contribution'=>'nullable|numeric|min:0',
            'expected_annual_return'=>'required|numeric|min:0|max:30','inflation_rate'=>'required|numeric|min:0|max:30',
            'desired_monthly_retirement_income'=>'nullable|numeric|min:0','retirement_years'=>'required|integer|min:1|max:60',
        ]);
        $profile = FinancialPlannerProfile::updateOrCreate(['user_id'=>$request->user()->id], $data);
